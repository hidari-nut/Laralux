...
@section('content')
    ...
    <div class="container my-4">
        <button onclick="window.location.href='{{ route('hotelTypes') }}'" class="btn btn-warning mb-3">View Types List</button>
        <div class='table-responsive'>
            <table class='table'>
                ...
                <tbody>
                    @foreach ($trashedTypes  as $hotelTypes)
                        <tr>
                            <th>{{ $hotelTypes->id }}</th>
                            <th>{{ $hotelTypes->name }}</th>
                            <td>
                                <button class="btn btn-success" data-toggle="modal" data-target="#restoreTypeModal" data-id="{{ $hotelTypes->id }}">Restore</button>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
    ...
    <div class="modal fade" id="restoreTypeModal" tabindex="-1" role="dialog" aria-labelledby="restoreTypeModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form method="POST" action="{{ route('hotelTypesRestore') }}">
                    @csrf
                    ...
                    <div class="modal-body">
                        Are you sure you want to restore this hotel type?
                    </div>
